...
import { useAnuncioAuditoria } from '../hooks/useAnunciosAuditoria.js';
...
const DashboardLayout = ({ anuncios, onAnuncioCreado, rolUsuario, loading }) => {
  ...
  const { auditoriaData, loading: loadingAuditoria, error: errorAuditoria, cargarAuditoria, limpiarAuditoria } = useAnuncioAuditoria();
  const [modalAuditoriaAbierto, setModalAuditoriaAbierto] = useState(false);

  const handleVerAuditoria = useCallback((id) => {
    cargarAuditoria(id);
